...
public class ValueAnswersFacade extends AbstractFacade<ValueAnswers, Integer> {
	...
	public List<ValueAnswers> buscarPorAvanceEjecucionGenero(int codigoAvanceEjecucion) throws Exception{
		String sql="SELECT VA FROM ValueAnswers VA WHERE VA.advanceExecutionSaveguards.adexId=:codigoAvanceEjecucion AND VA.advanceExecutionSaveguards.adexStatus=TRUE AND VA.advanceExecutionSaveguards.adexIsGender = TRUE ORDER BY VA.questions.quesQuestionOrder";
		Map<String, Object> camposCondicion=new HashMap<String, Object>();
		camposCondicion.put("codigoAvanceEjecucion", codigoAvanceEjecucion);
		return findByCreateQuery(sql, camposCondicion);
	}
