...
public class ProjectGenderIndicatorFacade extends AbstractFacade<ProjectGenderIndicator, Integer>{
	...
	public List<ProjectGenderIndicator> listaLineasGeneroProyectoPartner(Integer codigoProyecto,Integer codigoPartner) throws Exception{
		...
		if(codigoProyecto>0 && codigoPartner>0){
			sql="SELECT PGI from ProjectGenderIndicator PGI WHERE PGI.pgigStatus=TRUE AND PGI.projectsGenderInfo.projects.projId=:codigoProyecto AND PGI.projectsGenderInfo.projectsStrategicPartners.pspaId=:codigoPartner ORDER BY PGI.projectsGenderInfo.cataId.cataNumber";
			camposCondicion.put("codigoProyecto", codigoProyecto);		
			camposCondicion.put("codigoPartner", codigoPartner);
		}else{
			sql="SELECT PGI from ProjectGenderIndicator PGI WHERE PGI.pgigStatus=TRUE AND PGI.projectsGenderInfo.projects.projId=:codigoProyecto AND PGI.projectsGenderInfo.projectsStrategicPartners.pspaId IS NULL ORDER BY PGI.projectsGenderInfo.cataId.cataNumber";
			camposCondicion.put("codigoProyecto", codigoProyecto);		
		}		
