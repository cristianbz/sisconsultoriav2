...
import ec.gob.ambiente.sigma.ejb.sis.dto.DtoPreguntasPartners;
import ec.gob.ambiente.sigma.ejb.sis.dto.DtoResumenAbordajeSalvaguarda;
...
import ec.gob.ambiente.sigma.web.sis.beans.ReportesBean;
import ec.gob.ambiente.sigma.web.sis.utils.GeneradorPdfHtml;
import ec.gob.ambiente.sigma.web.sis.utils.GenerarPdfResumenAbordaje;
import ec.gob.ambiente.sigma.web.sis.utils.reporteria.DatosConsolidado;
import ec.gob.ambiente.sigma.web.sis.utils.reporteria.GenerarPdfConsolidado;
...
public class ReportesController implements Serializable{
	...
	public void init(){
		try{
			...
			getReportesBean().setListaSalvaguardas(new ArrayList<Safeguard>());
			getReportesBean().setListaSalvaguardas(getSafeguardsFacade().cargaSalvaguardasActivas());
			getReportesBean().setListaProyectos(new ArrayList<Project>());
	...
	public String llenaInformacionSocioEstrategico(List<AdvanceSummary> lista, String salvaguarda,String resumen){
		...
		for (AdvanceSummary as : lista) {				
			if(as.getSafeguards().getSafeCode().equals(salvaguarda) && as.getAdvanceExecutionSafeguards().getProjectsStrategicPartners()!=null){
				...
				if(avance==null)
					avance="";
				filas += "<tr><td class='tablaborder' bgcolor='#FFFFFF' >" + as.getAdvanceExecutionSafeguards().getProjectsStrategicPartners().getPartId().getPartName() + "</td> <td class='tablaborder' bgcolor='#FFFFFF' >" + avance + "</td> </tr>\r\n";							
			}			
		}
		filas += "</table>\r\n";
		resumen+=filas;
...
//			e.printStackTrace();
	...
	public void generarPdfResumenAbordajePorProyecto(){
		try{
			...
			dto.setSocioImplementador(getReportesBean().getProyectoSeleccionado().getPartId().getPartName());
...
//			e.printStackTrace();
	...
	public void generarPdfResumenAbordajePorSalvaguarda(){
		try{
			Map<Integer,Project> mapaTemp=new HashMap<Integer,Project>();
	...
	public void llenarInformacionPorSalvaguarda(Map<Integer, Project> mapa,List<AdvanceSummary> listaAvance,DtoResumenAbordajeSalvaguarda dto){
		String html="";
		for (Entry<Integer,Project> m : mapa.entrySet()){			
			...
			for (AdvanceSummary asu : listaAvance) {
				if(asu.getAdvanceExecutionSafeguards().getProjects().getProjId() == m.getKey()){
					if(asu.getAdvanceExecutionSafeguards().getProjectsStrategicPartners() != null){
						...
						if(avance==null)
							avance="";
						html += "<tr><td class='tablaborder' bgcolor='#FFFFFF' >" + asu.getAdvanceExecutionSafeguards().getProjectsStrategicPartners().getPartId().getPartName() + "</td> <td class='tablaborder' bgcolor='#FFFFFF' >" + avance + "</td> </tr>\r\n";
					}
				}
			}
			html+="</table><br/>\r\n";
